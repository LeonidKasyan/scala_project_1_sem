...
class AccountRepositoryMutable extends AccountRepository{
    ...
    override def createAccount(create: CreateAccount): Account = {
        val account = Account(id = UUID.randomUUID(), numberPhone = create.numberPhone, money = create.money)
    ...
    override def UpdateAccountMoney(update: UpdateAccountMoney): Option[Account] = {
        store.get(update.id).map {account=>
            
            val updated = account.copy(money = update.money) 
            store.put(account.id, updated)
            updated
        }  
    }
